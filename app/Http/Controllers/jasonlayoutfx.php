...
class jasonlayoutfx extends Controller{
...
function index(Request $request){
   ...
   $country=@$data->cityName.','.@$data->countryName;
    ...
    $page_id=base64_decode($request['MainM_ID']);
    $box_id=base64_decode($request['box_ID']);
    ...
    if($request['search_bt']){
      ...
      //nav-optimize
      $nav=nav::where('parent_id',NULL) ->where('SUBMENUID',NULL)->get();
      ...
      searchbar::create([
       ...
       $All='{
        "social": '.$social.',
        "Maxcompanyid": '.$Maxcompanyid.',
        "navNullfooter": '.$navNullfooter.',
        "nav": '.$nav.',
        "Keywords":'.$Keywords.',
        "nav": '.$nav.',
        "nav_sub":'.$nav_sub.',
  ...
  $static=SNumbers::where ('nav_id',1)->get();
   ...
   // $company=company::where ('nav_id',1)->get();
    ...
    $topic=topic::where ('nav_id',$page_id)->get();
     ...
     $Maxcompany_id=company::Max('id')->get();
      ...
      // $sliderMin=slider::whereRaw('id = (select Min(id)from sliders)')->where('nav_id',$page_id)->get();

      // $sliderMax=slider::whereRaw('id > (select Min(id)from sliders )')->where('nav_id',$page_id)->get();
   ...
   $static=SNumbers::where ('relatedtopic_id',$Topic_id)->get();
    ...
    $topicsbreadcrumb=topic::where('id',$Topic_id)->get();

    foreach($topicsbreadcrumb as  $breadcrumbitem ){
